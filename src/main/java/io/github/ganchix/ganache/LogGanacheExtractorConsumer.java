...
public class LogGanacheExtractorConsumer implements Consumer<OutputFrame> {
    ...
    private static final Pattern IS_ADDRESS_OR_PRIVATE_KEY = Pattern.compile("^\\((\\d{1,})\\) 0x.*([^(.*)])");
    ...
    public void accept(OutputFrame outputFrame) {
        ...
        if (utf8String != null) {
            ...
            if (IS_ADDRESS_OR_PRIVATE_KEY.matcher(message).matches()) {
                ...
                String privateKey = split[1].trim();
                String position = split[0].replaceFirst("\\(", "").replaceFirst("\\)", "");
                if (privateKey.startsWith("0x")) {
                    ganacheContainer.addAccountPrivateKey(Integer.parseInt(position), privateKey);
                }
            }
